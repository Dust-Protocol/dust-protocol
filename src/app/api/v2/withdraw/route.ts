    ...
    try {
      ...
      try {
        if (await isSanctioned(recipient)) {
          console.warn(`[V2/withdraw] Chainalysis: sanctioned recipient=${recipient}`)
          return NextResponse.json(
        ...
        console.warn('[V2/withdraw] Chainalysis API error (continuing):', apiErr)
      }
      ...
      if (receipt.status === 0) {
        throw new Error(`Withdraw transaction reverted: ${receipt.transactionHash}`)
      }
