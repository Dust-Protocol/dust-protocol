...
export function Navbar() {
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-50 h-16 bg-[#06080F]/95 backdrop-blur-md border-b border-white/[0.04] flex items-center px-4 lg:px-8">
        ...
        <div className="flex-1 flex items-center min-w-0">
          <Link href="/dashboard" className="flex items-center gap-2.5 shrink-0 group">
            <DustLogo size={24} color="#00EF8B" className="group-hover:drop-shadow-[0_0_8px_rgba(0,239,139,0.6)] transition-all duration-300" />
            <span className="flex items-baseline gap-2">
              <span className="text-base font-bold tracking-[0.2em] text-white font-mono group-hover:text-[#00EF8B] transition-colors duration-300">DUST</span>
              <span className="hidden sm:inline text-[10px] font-mono tracking-[0.3em] text-[#00EF8B]/50 uppercase group-hover:text-[#00EF8B]/80 transition-colors duration-300">PROTOCOL</span>
            </span>
          </Link>
        </div>
        ...
        <div className="hidden lg:flex items-center gap-1 xl:gap-1.5 shrink-0">
          {isConnected && navItems.map(item => {
            ...
            return (
              ...
              >
                {isActive && (
                  <span className="absolute bottom-0 left-2 right-2 h-[1px] bg-[#00EF8B]/70" />
                )}
        ...
        <div className="flex-1 flex items-center justify-end gap-2 sm:gap-3 min-w-0">
          ...
          {isConnected && (
            <div className="relative shrink-0" ref={chainBadgeRef}>
              {chains.length === 1 ? (
                <div className="flex items-center gap-1.5 sm:gap-2 px-2 sm:px-2.5 py-1.5 sm:py-2 rounded-sm border border-white/10 bg-[#0a0d14] shadow-sm">
                ...
                <>
                  <button
                    ...
                    className="flex items-center gap-1.5 sm:gap-2 px-2 sm:px-2.5 py-1.5 sm:py-2 rounded-sm border border-white/10 bg-[#0a0d14] hover:bg-[#0a0d14]/80 hover:border-[#00EF8B]/40 transition-all duration-300 group shadow-sm"
                  >
                    <ChainTokenIcon size={16} chainId={activeChain.id} />
                    <span className="hidden sm:inline text-[11px] font-mono text-white/70 group-hover:text-[#00EF8B] transition-colors tracking-wide">
                      {shortChainName(activeChain.name).toUpperCase()}
                    </span>
                    <ChevronDownIcon
                      className="w-3 h-3 text-white/30 group-hover:text-[#00EF8B]/70 transition-all duration-300"
                      style={{ transform: chainDropdownOpen ? "rotate(180deg)" : "none" }}
                    />
                  </button>

                  {chainDropdownOpen && (
                    <div className="absolute top-[calc(100%+8px)] right-0 bg-[#0a0d14] border border-[#00EF8B]/20 rounded-sm min-w-[220px] z-50 overflow-hidden shadow-[0_8px_32px_-4px_rgba(0,0,0,0.8)] animate-in fade-in slide-in-from-top-1 duration-200">
                      <div className="px-4 pt-3 pb-2 border-b border-white/[0.04]">
                        <span className="text-[10px] font-mono text-[#00EF8B]/50 tracking-[0.2em] uppercase">Network</span>
                      </div>
                      <div className="py-1">
                        {chains.map(chain => {
                          ...
                          return (
                            <button
                              ...
                              className={`w-full text-left px-4 py-2.5 text-xs font-mono transition-all flex items-center gap-3 relative group ${isActive
                                  ? 'text-[#00EF8B] bg-[#00EF8B]/[0.08]'
                                  : 'text-white/60 hover:bg-white/[0.04] hover:text-white'
                                }`}
                            >
                              {isActive && <div className="absolute left-0 top-0 bottom-0 w-[2px] bg-[#00EF8B] shadow-[0_0_8px_rgba(0,239,139,1)]" />}
                              <ChainTokenIcon size={16} chainId={chain.id} />
                              <span className="flex-1 tracking-wide">{chain.name}</span>
                              {isActive && <span className="w-1.5 h-1.5 bg-[#00EF8B] shadow-[0_0_6px_rgba(0,239,139,0.8)] rounded-sm"></span>}
                            </button>
                          );
                        })}
                      </div>
            ...
            {displayName ? (
              <>
                <button
                  ...
                  className="flex items-center gap-2 px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-sm border border-white/10 bg-[#0a0d14] hover:bg-[#0a0d14]/80 hover:border-[#00EF8B]/40 transition-all duration-300 group shadow-sm"
                >
                  <div className="w-1.5 h-1.5 bg-[#00EF8B] animate-pulse shrink-0 rounded-sm" />
                  <span className="hidden sm:inline text-xs font-mono text-white/90 group-hover:text-[#00EF8B] transition-colors">{displayName}</span>
                  <ChevronDownIcon
                    className="w-3.5 h-3.5 text-white/40 group-hover:text-[#00EF8B]/70 transition-all duration-300"
                ...
                {walletOpen && (
                  <div className="absolute top-[calc(100%+8px)] right-0 bg-[#0a0d14] border border-[#00EF8B]/20 rounded-sm min-w-[220px] z-50 overflow-hidden shadow-[0_8px_32px_-4px_rgba(0,0,0,0.8)] animate-in fade-in slide-in-from-top-1 duration-200">
                    <div className="py-1">
                      ...
                      >
                        {copied ? <CheckIcon className="w-4 h-4 text-[#00EF8B]" /> : <CopyIcon className="w-4 h-4" />}
              ...
              <button
                ...
                className="relative px-5 py-2 text-[11px] font-mono font-bold tracking-widest text-[#00EF8B] border border-[#00EF8B]/30 bg-[#00EF8B]/[0.05] rounded-sm hover:border-[#00EF8B] hover:bg-[#00EF8B]/[0.1] hover:shadow-[0_0_12px_rgba(0,239,139,0.2)] transition-all duration-300 group overflow-hidden"
              >
                <div className="absolute inset-0 w-[200%] bg-gradient-to-r from-transparent via-[#00EF8B]/20 to-transparent -translate-x-[100%] group-hover:translate-x-[100%] transition-transform duration-700 ease-in-out" />
      ...
      {/* Mobile nav drawer */}
      {mobileOpen && (
        <div className="fixed top-16 left-0 right-0 z-40 bg-[#0a0d14]/95 backdrop-blur-xl border-b border-[#00EF8B]/20 flex flex-col py-3 shadow-[0_8px_32px_rgba(0,0,0,0.8)] animate-in slide-in-from-top-2 duration-200">
          {/* Mobile chain switcher — hidden when single chain */}
          {isConnected && chains.length > 1 && (
            <div className="px-3 mb-2 pb-2 border-b border-white/[0.04]">
              <span className="block px-4 pb-2 text-[10px] font-mono text-[#00EF8B]/50 tracking-[0.2em] uppercase">Network</span>
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-3 px-4 py-2.5 text-xs font-mono tracking-widest transition-all rounded-sm relative ${isActive
                          ? 'text-[#00EF8B] bg-[#00EF8B]/[0.08] border-l-2 border-[#00EF8B]'
          ...
          <div className="flex flex-col gap-0.5 px-3">
            {isConnected ? navItems.map(item => {
              ...
              <button
                ...
                className="mx-1 mt-2 py-3.5 relative text-xs font-mono font-bold tracking-widest text-[#00EF8B] border border-[#00EF8B]/30 bg-[#00EF8B]/[0.05] rounded-sm hover:border-[#00EF8B] hover:bg-[#00EF8B]/[0.1] transition-all flex items-center justify-center shadow-[0_0_12px_rgba(0,239,139,0.1)] overflow-hidden group"
              >
                <div className="absolute inset-0 w-[200%] bg-gradient-to-r from-transparent via-[#00EF8B]/20 to-transparent -translate-x-[100%] group-hover:translate-x-[100%] transition-transform duration-700 ease-in-out" />
                <span className="relative">CONNECT WALLET</span>
              </button>
